...
pqAsyncWorker::pqAsyncWorker(Function& callback, std::string sql, std::vector<std::string> params)
        : AsyncWorker(callback), sql(sql), params(params) {simple_mode = false;};
pqAsyncWorker::pqAsyncWorker(Function& callback, std::string sql)
        : AsyncWorker(callback), sql(sql) {simple_mode = true;};
